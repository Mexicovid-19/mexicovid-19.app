...
router.post('/data/calculadora',function(req,res){
    ...
    let amortizacion =()=>{
        ...
        var d3 = require('d3');
        var amortizacion;
        fs.readFile("data/calculator/Amortizacion.csv","utf8",function(err,data){
            if (err) {
    ...
    let deficit =()=>{
        ...
        var d3 = require('d3');
        var deficit;
        fs.readFile("data/calculator/deficit.csv","utf8",function(err,data){
            if (err) {
                res.statusMessage = `File doesn't exist or couldn't open`;
                return res.status(409).end();
            }
    ...
    let demandaAgregada =()=>{
        ...
        var d3 = require('d3');
        var demandaAgregada;
        fs.readFile("data/calculator/demandaAgregada.csv","utf8",function(err,data){
            if (err) {
                res.statusMessage = `File doesn't exist or couldn't open`;
                return res.status(409).end();
            }
    ...
    let finanzasPublicas=()=>{
        ...
        fs.readFile("data/calculator/FinanzasPublicas.csv","utf8",function(err,data){
            if (err) {
                res.statusMessage = `File doesn't exist or couldn't open`;
                return res.status(409).end();
            }
    ...
    let parametrosDeudaMxn=()=>{
        ...
        fs.readFile("data/calculator/ParametrosDeudaMxn.csv","utf8",function(err,data){
            if (err) {
                res.statusMessage = `File doesn't exist or couldn't open`;
                return res.status(409).end();
            }
    ...
    let pibNominal=()=>{
        ...
        var d3 = require('d3');
        var PIBNominal;
        fs.readFile("data/calculator/PIBNominal.csv","utf8",function(err,data){
            if (err) {
                res.statusMessage = `File doesn't exist or couldn't open`;
                return res.status(409).end();
            }
    ...
    let tasaInteres=()=>{
        ...
        var d3 = require('d3');
        var TasaInteres;
        fs.readFile("data/calculator/TasaInteres.csv","utf8",function(err,data){
            if (err) {
                res.statusMessage = `File doesn't exist or couldn't open`;
                return res.status(409).end();
            }
    ...
    let tipoCambio=()=>{
        ...
        var d3 = require('d3');
        var TipoCambio;
        fs.readFile("data/calculator/TipoCambio.csv","utf8",function(err,data){
            if (err) {
                res.statusMessage = `File doesn't exist or couldn't open`;
                return res.status(409).end();
            }
    ...
    let pibMxn=()=>{
        ...
        var d3 = require('d3');
        var PIBMxn;
        fs.readFile("data/calculator/PIBMxn.csv","utf8",function(err,data){
            if (err) {
                res.statusMessage = `File doesn't exist or couldn't open`;
                return res.status(409).end();
            }
            PIBMxn = d3.csvParse(data);
            var myjson = [];
            for (var i = 0; i < PIBMxn.length; i++){
                            
                var periodo = PIBMxn[i].PERIODO	
                var ano = PIBMxn[i].ANO	
                var pib = PIBMxn[i].PIB	
                var da = PIBMxn[i].DA	
                var c = PIBMxn[i].C	
                var g = PIBMxn[i].G	
                var ii = PIBMxn[i].I	
                var inv = PIBMxn[i].INV	
                var xm = PIBMxn[i].XM	
                var x = PIBMxn[i].X	
                var m = PIBMxn[i].M	
                var e = PIBMxn[i].E

                delete PIBMxn[i].PERIODO
                delete PIBMxn[i].ANO	
                delete PIBMxn[i].PIB
                delete PIBMxn[i].DA
                delete PIBMxn[i].C	
