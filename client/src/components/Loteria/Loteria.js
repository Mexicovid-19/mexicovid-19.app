...
const handleChange = (event) => {
    ...
    setGraphData({ ...graphData, [event.target.name]: event.target.checked, nationalData:casos, tipoDatos: 'Casos Nacionales Activos' });
    }
    else{
        setGraphData({ ...graphData, [event.target.name]: event.target.checked, nationalData:muertes, tipoDatos: 'Defunciones Nacionales' });
    }
  };
  ...
  return (
    <div >
      ...
      <div className={classes.tablero}>
        ...
        <h2 className={classes.subtitle}><strong> {graphData.tipoDatos} Gráfica</strong></h2>
        </div>
          <NationalGraph data={graphData.nationalData} />
        <FormGroup row>
      <FormControlLabel
        control={<Switch checked={graphData.checkedA} onChange={handleChange} name="checkedA" />}
